	...
	//var welMultilpleUploadInputBox = jindo.$$.getSingle("#customfiles",elContent);
	var welMultilpleUploadInputBox = $("customfiles");
	...
	//--------------------- html5  지원되는 브라우저에서 사용하는 함수  --------------------------
 	...
 	function multiFileBtn(){
 		var tmp = welMultilpleUploadInputBox;
		tmp.click();
		tmp.onchange = function(){
			...
			for (var i = 0, j = nImageFileCount ; i < nCount ; i++){
				if (!rFilter.test(files[i].type)) {
					...
					alert("이미지 용량이 3MB를 초과하여 등록할 수 없습니다.");
				} else {
					//제한된 수만 업로드 가능.
					if ( j < nMaxImageCount ){
    ...
    function addImage(ofile){
		...
		if( !!bExceedLimitTotalSize ){
			alert("전체 이미지 용량이 10MB를 초과하여 등록할 수 없습니다. \n\n (파일명 : "+sFileName+", 사이즈 : "+sFileSize+")");
		} else {
			//이미지 정보 저장							
    ...
    function html5Upload() {	
    	...
    	sUploadURL= '/common/editor/popup/quick_photo/FileUploader_html5.php'; 	//upload URL
    	...
    	for(var j=0, k=0; j < nImageInfoCnt; j++) {
    		...
    		try{
	    		if(!!tempFile){
	    			...
	    			callAjaxForHTML5(tempFile,sUploadURL, urlParam['path']);
	    			k += 1;
	    		}
	    	}catch(e){}
    		tempFile = null;
    ...
    function callAjaxForHTML5 (tempFile, sUploadURL, filepath){
		...
		oAjax.header("stdFilePath",filepath);
	...
	function addEvent() {
		...
		//elDropArea.addEventListener("click", click, false);
	...
	function removeEvent(){
	    ...
	    //elDropArea.removeEventListener("click", click, false);
	}
	...
	function onAjaxError (e){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : '/common/editor/popup/quick_photo/FileUploader.php',	//샘플 URL입니다.
